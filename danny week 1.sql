...
(
		SELECT S.customer_id
			,M.product_name
			,(S.order_date)
			,DATEADD(day, 7, S.order_date) AS [1 Week]
			,COUNT(S.product_id) as [Count of Product]
			,SUM(price) as [Total Spend]
		FROM dbo.sales S
		LEFT JOIN dbo.menu M
...
SELECT Spend.customer_id
		,Spend.order_date
		,[Points] = (CASE WHEN Spend.order_date >= MIN(Spend.order_date) AND Spend.order_date <= Spend.[1 Week] THEN (Spend.[Total Spend]*2)
						ELSE Spend.[Total Spend]
						END)
FROM Spend
